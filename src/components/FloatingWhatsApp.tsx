...
export const FloatingWhatsApp = () => {
  ...
  useEffect(() => {
    const handleBottomBarVisibility = (e: CustomEvent<{ visible: boolean }>) => {
      setHideOnMobile(e.detail.visible);
    };

    window.addEventListener('mobileBottomBarVisibility', handleBottomBarVisibility as EventListener);
    return () => window.removeEventListener('mobileBottomBarVisibility', handleBottomBarVisibility as EventListener);
  }, []);
  ...
  return (
    <div className={`fixed bottom-24 right-4 md:bottom-8 md:right-8 z-40 flex flex-col items-end gap-1 animate-scale-in ${hideOnMobile ? 'hidden md:flex' : ''}`}>
      <button
